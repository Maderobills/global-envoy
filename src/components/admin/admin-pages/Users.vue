  ...
  <div class="h-4/5 bg-slate-100 flex flex-col rounded-lg">
    ...
    <div v-if="loading" class="space-y-4 mx-6">
      <div v-for="n in 3" :key="n" class="animate-pulse">
        <div class="pl-10">
          <div
            class="h-8 bg-slate-300 rounded-lg w-60 border-x-4 border-slate-600"
          ></div>
        </div>
      </div>
    </div>
    ...
    <div v-else class="space-y-2">
      ...
      >
        <Username
          v-for="user in users.filter((user) => user && user.id)"
          :key="user.id"
...
const refreshUsers = () => {
  ...
  refreshTimeout = setTimeout(() => {
    ...
    fetchUsers(); // This will now merge new data
  }, 300);
};
...
const handleUserClick = (user) => {
  if (!user) {
    ...
    return; // Exit if user is undefined or null
  }
