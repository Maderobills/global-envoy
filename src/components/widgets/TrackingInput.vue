<template>
  <div :style="backgroundImageStyle" class="bg-cover bg-center h-80 flex flex-col justify-between">
    <div class="h-full text-white text-center p-6 bg-black bg-opacity-20 hover:backdrop-blur-sm space-y-6">
      ...
      <input v-model="message" placeholder="Enter your tracking number" class="rounded-s h-12 w-96 p-5 text-slate-900 text-md outline-none">
      <button class="transition bg-slate-900 px-10 font-semibold hover:font-bold rounded shadow-slate-900 hover:shadow-md">Track</button>
    </div>
